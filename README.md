...
| `users[].password` | string | The password of this user. This field is for providing the password in its cleartext form. It is recommended to use the `password_hash` field instead to avoid leaking secrets to other users that can read the seed file. This field may not be provided if `password_hash` is provided. |
| `users[].password_hash` | string | The password of this user, as a hash generated by libxcrypt. Values for this field can be obtained interactively with the command line tool `mkpasswd` that is available on most Linux distributions (sometimes as part of the `whois` package). This field may not be provided if `password` is provided. |
